...
def shift():    # +- 20% 이동
    for npy_file in os.listdir(folder_path):
        ...
        for x in range(0, 250, 4):
            nonzero_indices = np.nonzero(data[:, x])[0]  # 0이 아닌 값의 인덱스를 찾기
            if len(nonzero_indices) > 0:  # 0이 아닌 값이 하나라도 있는 경우에만 연산을 수행
                data[:, x][nonzero_indices] = data[:, x][nonzero_indices] * 0.8
        
        # 1번 인덱스부터 250번 인덱스까지
        # (y 좌표값) * (이동시킬 퍼센테이지)
        for y in range(1, 251, 4):
            nonzero_indices = np.nonzero(data[:, y])[0]  # 0이 아닌 값의 인덱스를 찾기
            if len(nonzero_indices) > 0:  # 0이 아닌 값이 하나라도 있는 경우에만 연산을 수행
                data[:, y][nonzero_indices] = data[:, y][nonzero_indices] * 0.8
